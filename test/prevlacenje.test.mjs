...
describe('Zamucenja po pravcu kretanja NEMA', () => {
  /* Postojalo je u v264: vodoravni SVG filter na obe stranice, jacine vezane za
     brzinu. U pregledacu, i sa osmostruko usporenim procesorom, nije kostao
     nijedan ispusten kadar - a na telefonu je seckalo. Ova zamka nije o ukusu
     nego o tome da se ne vrati bez merenja na pravom uredjaju: iznad sadrzaja
     koji se pomera stoje zaglavlje i traka tabova sa `backdrop-filter`, i
     filter preko pokretnog sloja im obara jeftin put. */
  test('nijedna stranica ne nosi filter dok se pomera', () => {
    const html = readRepoFile('index.html');
    assert.ok(!/feGaussianBlur/.test(html), 'filter za zamucenje je vracen u index.html');
    assert.ok(!/filter:url/.test(html), 'neka stranica opet dobija filter u pokretu');
    assert.ok(!/pvMutno|pvMutniPad/.test(readRepoFile('app.js')),
      'kod za zamucenje je vracen u app.js');
  });
});
